...
当前结构化 JSON 中的 42 条案例均已配套 `gpt-image-2` 生成结果图，其中包含 18 条参考 CC0 社区项目改写的案例。图片保存在 `assets/cases/`，并在 JSON 数据的 `outputs` 字段中记录模型、来源、许可和备注；社区改写案例另在 `reference` 字段记录原项目、链接、许可和改写说明。下表展示每个分类的代表案例，更多结果图见各分类案例页。
